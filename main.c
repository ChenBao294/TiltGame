...
/*Function which parses the value of the acceleration, which comes in the form "x(xvalue)y(yvalue)"*/
void parse_acceleration(char *buff, float *x, float *y){
  char * pch;
  pch = strtok (buff,"xy");
  *x = atof(pch);
  pch = strtok (NULL, "xy");
...
/*Function which reads the acceleration values transmitted serially, parses them, calculates the corresponding angles, and updates the position of the platform */   
void *RedBear_controller( void *ptr )
{
  //get value of the serial port
  int *fd;
  fd = (int*) ptr;

  //declarations of angle variables
  float xangs[10];
  float yangs[10];
  float xang = 0.0, yang = 0.0;

  //initializations of these variables
  for (int i=0; i<10; i++){
    xangs[i] = 0.0;
    yangs[i] = 0.0;
  }
  ...
  //main loop
  while(1){
    char buffer[20]; //read buffer
    int n; //return value of read function
    float nxang = 0.0, nyang = 0.0; //new values of the angles
    n = read(*fd, buffer, sizeof(buffer)); //read the acceleration values
    
    if (n >= 0){
      parse_acceleration(buffer,&nxang,&nyang); //parse the acceleration values
      nxang = -asin(nxang/9.8)*180/3.14; //determine the angles
      nyang = -asin(nyang/9.8)*180/3.14;
      
      if(!isnan(nxang) && !isnan(nyang)){ //if these angles are defined
	xang = 0.0;
	yang = 0.0;

	//the following code smoothens the change to xang and yang by taking the average value of the new angles with the 10 previous values of the angles
	...
	//Set the new orientation of the platform
	st_angle(xang,yang);
...
int main(int argc, char *argv[])
{
    ...
    /*create new thread which calls the RedBear_controller function*/
    pthread_t redbearthread;
    int rth;
    rth = pthread_create( &redbearthread, NULL, RedBear_controller, (void*) &fd);
    if(rth)
    {
